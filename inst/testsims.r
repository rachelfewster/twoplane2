...
# TODO: correct arguments to sim.2plane
...
#  estsnspp$Dhat[sim]=est.nspp$pars["D"]
  estsnspp$Dhat[sim]=est.nspp[1]  # $pars["D.2D"]
  estsnspp$sigma[sim]=est.nspp[3] # $pars["sigma"]
...
#  Percentage bias
(mean(estsio$Dhat)/mean(true$D))*100
hist(estsio$Dhat)

mvs = sdat$alls1-sdat$alls2
hist(diff(mvs)*planespd*1000/k)
mean(abs(diff(mvs)*planespd*1000/k))
