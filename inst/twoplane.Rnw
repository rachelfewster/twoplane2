...
\begin{abstract}
Line transect aerial surveys are a relatively cheap way of surveying marine mammal populations, and we anticipate that will increasingly be done by unmanned aerial vehicles. Because aircraft move quickly and marine mammals dive, these surveys often violate the conventional line transect assumption of certain detection of animals on the line. Here we develop a mark-recapture method for two independent observers that does not require recapture identification. More specifically, we develop and test a maximum likelihood estimation method designed for surveys with two high-resolution digital cameras in place of two human observers. It requires that detections of species of interest be identified on each camera, but does not require recapture identification. It also requires an assumption about mean dive cycle length, and a relatively weak assumption about the form of the animal movement processes. In addition to estimating density, the method estimates the proportion of time animals are available, and the mean speed of animal movement.

We estimate density using the data analysed by Stevenson, Borchers and Fewster (2018) ``Cluster capture-recapture to account for identification uncertainty on aerial surveys of animal populations'' and test the method by simulation. The new method is found to be unbiased, to have coverage probabilities close their nominal value and to be slightly more precise than the cluster capture-recapture method. We discuss briefly possible extensions to the method.
\end{abstract}
