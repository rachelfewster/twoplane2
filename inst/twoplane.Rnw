...
We estimated the variances of parameters using the inverse of the Hessian obtained in the fitting process. Confidence intervals for the parameters $D$, $\sigma$ and $\kappa$ were obtained using the inverse log transformation of confidence intervals for $\theta$, $\phi$ and $\alpha_\gamma$, assuming normality of the maximum likelihood estimators of these parameters. 
...
Like \cite{Stevenson+al:18}, we use a searched strip half-width of $W=0.125$ km and a ``buffer'' of $b=2$ km beyond this, beyond which we assume no animal could enter the searched strip between the fist and second observer passing it. They  obtained the following estimates (95\% confidence intervals in brackets): $\hat{D}=1.05$ (0.84, 1.60) pods per km2, $\hat{\sigma}_{palm}=0.15$ km, and the expected proportion of time in the surface state $\hat{\gamma}=0.86$ (0.56, 1.00)). The corresponding estimates using our method are $\hat{D}=\Sexpr{round(mle["D","est"],2)}$ (\Sexpr{round(mle["D","lo"],2)},\Sexpr{round(mle["D","hi"],2)}) pods per km2, $\hat{\sigma}_{248}=\hat{\sigma}\sqrt{248}=\Sexpr{round(mle["sigma","est"],2)}$ (\Sexpr{round(mle["sigma","lo"],2)},\Sexpr{round(mle["sigma","hi"],2)}) km, and $\hat{\gamma}=\hat{\kappa}/\tau=\Sexpr{round(mle["mu1","est"]/100,2)}$ (\Sexpr{round(mle["mu1","lo"]/100,2)},\Sexpr{round(mle["mu1","hi"]/100,2)}). The estimate $\hat{\sigma}_{248}=\Sexpr{round(mle["sigma","est"],2)}$ corresponds to a mean rate of movement of $\Sexpr{round(speeds["mle","meanspd"],2)}$ m/s.

\cite{Stevenson+al:18} estimated the coefficients of variation (CVs) of $\hat{D}$, $\hat{\sigma}_{palm}$ and $\hat{\gamma}$ to be $19$\%, $16$\% and $13$\%, respectively. The corresponding estimated CVs from our method (involving $\hat{\sigma}_{248}$ instead of $\hat{\sigma}_{palm}$) are $\Sexpr{round(mle["D","cv"]*100)}$\%, $\Sexpr{round(mle["sigma","cv"]*100)}$\% and $\Sexpr{round(mle["mu1","cv"]*100)}$\%, respectively.
...
simtab <- readRDS("./results/simtab_En100_30.Rds")
mlesimtab = simtab[,c(2:4,6:8,12:13)]
names(mlesimtab) = c("gamma","l","speed","\\%Bias","\\%CV","\\%Cover","mean(n)","mean(m)")
